...
define(["jquery", "bootstrap"], function() {
	...
	base.modal = function(option) {
		...
		self.background = option.background ? option.background : "transparent";
		self.modalLabelId = option.id + "label";
		self.style = option.style ? option.style : "";
		self.width = option.width ? option.width : null;
		self.visible = option.visible==true ? true : false;
		self.center = option.center==false ? false : true;
		self.drag = option.drag==false ? false : true;
		...
		self.callback = option.callback ? option.callback : null;
		self.customScroll = option.customScroll==false ? false : true;
	...
	base.scroll = function(option) {
		...
		if(option.scrollStartCallback){
			self.scrollOption.callbacks.onScrollStart = option.scrollStartCallback;
		}
		if(option.scrollCallback){
			self.scrollOption.callbacks.onScroll = option.scrollCallback;
		}
		self.create = function() {
	...
	/**全选checkbox**/
	base.selectAll = function(e1, e2,callback, hasSkin) {
		if(hasSkin){
			var skin1= $(e1).next();
			var skin2 = $(e2).next();
			
			$(skin1).unbind("click").on("click",function(){
				$(e1).click();
				if($(e1).is(':checked')) {
					$(e2).not(":disabled").prop("checked", true);
					$(skin1).attr("class",$(skin1).attr("activeicon"));
					$(skin1).css("color",base.checkboxColor[0]);
					$(skin2).attr("class",$(skin2).attr("activeicon"));
					$(skin2).css("color",base.checkboxColor[0]);
				} else {
					$(e2).not(":disabled").prop("checked", false);
					$(skin1).attr("class",$(skin2).attr("icon"));
					$(skin1).css("color",base.checkboxColor[1]);
					$(skin2).attr("class",$(skin2).attr("icon"));
					$(skin2).css("color",base.checkboxColor[1]);
				}
				if(callback) {
					callback(e1, e2);
				}
			});
			$(skin2).unbind("click").on("click",function(){
				var selfSkin = $(this);
				selfSkin.prev().click();
				var activeIcon = $(skin1).attr("activeicon");
				var icon = $(skin1).attr("icon");
				if(selfSkin.prev().is(":checked")){
					selfSkin.attr("class",activeIcon);
					selfSkin.css("color",base.checkboxColor[0]);
				}else{
					selfSkin.attr("class",icon);
					selfSkin.css("color",base.checkboxColor[1]);
				}
				if($(e2).filter(":checked").length == $(e2).length) {
					$(e1).prop("checked", true);
					$(skin1).css("color",base.checkboxColor[0]);
					$(skin1).attr("class",activeIcon);
				} else {
					$(e1).prop("checked", false);
					$(skin1).attr("class",icon);
					$(skin1).css("color",base.checkboxColor[1]);
		...
		$(e1).unbind("click").on("click", function() {
			if($(this).is(':checked')) {
				$(e2).not(":disabled").prop("checked", true);
				if(hasSkin){
					$(e1).next().attr("class",$(skin1).attr("activeicon"));
					$(e2).next().attr("class",$(skin1).attr("activeicon"));
				}
			} else {
				$(e2).not(":disabled").prop("checked", false);
				if(hasSkin){
					$(e1).next().attr("class",$(skin1).attr("icon"));
					$(e2).next().attr("class",$(skin1).attr("icon"));
				}
			}
			if(callback) {
				callback(e1, e2);
		...
		$(e2).unbind("click").on("click", function() {
			if(hasSkin){
				var skin = $(this).next();
				if($(this).is(":checked")){
					skin.attr("class",skin.attr("activeicon"));
				}else{
					skin.attr("class",skin.attr("icon"));
				}
			}
			if($(e2).filter(":checked").length == $(e2).length) {
				$(e1).prop("checked", true);
	...
	base.formEntity = function() {
		var self = this;
		self.init = function(dataMap, form,defs,nullDef) {
			...
			for(var key in dataMap) {
				...
				if(defs&&defs[key]){
					...
					switch(type) {
						case "checkbox":
						case "radio":
							base.setChecks(val, e.attr("name"));
							break;
	...
	/**带皮肤的checkbox**/
	base.form.checkboxSkin = function(option){
		var self = {};
		self.checkbox = option.checkbox?option.checkbox:null;
		self.iconSize = option.iconSize?option.iconSize:18;
		self.icon = option.skinIcon?option.skinIcon:"fa fa-square-o";
		self.activeIcon = option.activeIcon?option.activeIcon:"fa fa-check-square";
		self.create = function(){
			//var txtShadow = "text-shadow:0 0 5px "+base.checkboxColor[1]+";";
			//var activeTxtShadow = "text-shadow:0 0 5px "+base.checkboxColor[0]+";";
			
			$(self.checkbox).each(function(i,o){
				$(o).hide();
				if($(o).next().attr("type")!="checkboxicon"){
					if($(o).is(':checked')){
						var s = "<i type='checkboxicon' style='color:"+base.checkboxColor[0]+";cursor:pointer;font-size:"+self.iconSize+"px'"+
						" val='"+$(o).val()+"'"+
						" icon='"+self.icon+"' activeicon='"+self.activeIcon+"' class='"+self.activeIcon+"'></i>";
						$(o).after(s);
					}else{
						$(o).after("<i type='checkboxicon' style='color:"+base.checkboxColor[1]+";cursor:pointer;font-size:"+self.iconSize+"px' val='"+$(o).val()+"' icon='"+self.icon+"' activeicon='"+self.activeIcon+"' class='"+self.icon+"'></i>");
					}
				}else{
					var skin = $(o).next();
					if($(o).is(":checked")){
						skin.attr("class",skin.attr("activeicon"));
						skin.css("color",base.checkboxColor[0]);
					}else{
						skin.attr("class",skin.attr("icon"));
						skin.css("color",base.checkboxColor[1]);
					}
	...
	base.form.element = function(option) {
		...
		self.keyDefs = option.keyDefs?option.keyDefs:null;
		self.element = null;
		self.create = function() {
			self.createElement();
			return self;
		...
		self.createElement = function(option) {
			...
			if(self.keyDefs&&self.keyDefs.key){
				key = self.keyDefs.key;
			}
			if(self.keyDefs&&self.keyDefs.value){
				value = self.keyDefs.value;
			}
			...
			switch(self.type) {
				...
				case "select":
					...
					var draw = function(data, optionData) {
						$(self.element).append("<option value='*'>*请选择*</option>");
						$(data).each(function(i, o) {
				...
				case "button":
					...
					self.element = base.form.button(self.data).element;
	...
	base.form.getParams = function(form, isGet) {
		...
		if(self.form&&$(self.form)[0]) {
			...
			for(var i = 0, j = eles.length; i < j; i++) {
				...
				} else if(o.tagName.toLowerCase() == "select" && $(o).val() == "*") {
					continue;
	...
	base.form.validate = function(option) {
		...
		self.error = function(role, errorText, obj) {
			switch(self.promptType) {
				...
				case "follow":
					...
					$(obj).parent().append("<div class='ui-form-error'>" + errorText + "</div>");
		...
		self.verify = function(obj, role) {
			...
			if($(obj).attr("type")){
				if($(obj).attr("type").toLowerCase()!="hidden"&&$(obj).is(":hidden")){return;}
			}
								...
								case "input":
									switch($(obj).attr("type")) {
										...
										case "radio":
											...
											if(base.getChecks($(obj).attr("name")).val.length == 0) {
												errorText = "必选项";
										...
										default:
											...
											if(!val) {
												...
												if(tsi.length>0){
													hasError = self.error(role, errorText, tsi[0]);
												}else{
													hasError = self.error(role, errorText, obj);
												}
	...
	base.form.button = function(option) {
		...
		self.create = function() {
			...
			if(self.disabled) {
				...
				if(self.clickEvent) {
					$(self.element).unbind("click").on("click", function() {
	...
	base.form.treeSelect = function(option) {
		...
		self.type = option.type ? option.type : "get";
		self.clickCallback = option.clickCallback ? option.clickCallback : null;
		self.checkCallback = option.checkCallback ? option.checkCallback : null;
		self.id = option.id ? option.id : base.getRandom(1000, 9999);
		self.selectNodeId = option.selectNodeId ? option.selectNodeId : null;
		self.treeObj = null;
		self.deviationLeft = option.deviationLeft ? option.deviationLeft:0;
		self.deviationTop = option.deviationTop ? option.deviationTop:0;
		self.scrollObj = option.scrollObj?option.scrollObj:null;
		self.position = option.position?option.position:"bottom";
		self.height = option.height?option.height:180;
			...
			self.setting = {
				...
				callback:{
					onCheck:function(event, treeId, treeNode){
						var checkNodes = self.treeObj.treeObj.getCheckedNodes(true);
						var v = "";
						var ids = "";
						$(checkNodes).each(function(i,o){
						...
						$(self.container).val(v);
						$(self.container).attr("ids",ids);
						if(self.checkCallback) {
							self.checkCallback(event, treeId, treeNode,checkNodes);
						}
					}
				}
			};
		...
		self.create = function() {
			if($(self.container).length > 0) {
				...
				$(self.container).attr("mt","treeSelectInput");
				$(self.container).css("cursor","pointer");
				$(self.container).on("click", function(e) {
					//$("div[type='treeSelect']").remove();
				...
				$(document).on("click", function(e) {
					...
					//if($(ele).attr("mt")=="treeSelectInput"){return;}
		...
		self.setPosition = function(){
			...
			switch(self.position){
				case "top"://往上方弹出
					if(self.scrollObj){
						var scrollTop = $(self.scrollObj.container).find(".mCSB_container").css("top");
						scrollTop = scrollTop?scrollTop:"0px";
						scrollTop = Number(scrollTop.replace(/px/g,""));
						left= $(self.container).offset().left - self.deviationLeft;
						top = $(self.container).offset().top - self.deviationTop - self.height - scrollTop;
					}else{
						left= $(self.container).offset().left - self.deviationLeft;
						top = $(self.container).offset().top - self.deviationTop - self.height;
					}
					$(self.treeContainer).css("left", left);
					$(self.treeContainer).css("top", top);
				break;
				
				case "bottom"://往下方弹出，默认
					if(self.scrollObj){
						var scrollTop = $(self.scrollObj.container).find(".mCSB_container").css("top");
						scrollTop = Number(scrollTop.replace(/px/g,""));
						left= $(self.container).offset().left - self.deviationLeft;
						top = $(self.container).offset().top + $(self.container).height() - self.deviationTop - scrollTop;
					}else{
						left= $(self.container).offset().left - self.deviationLeft;
						top = $(self.container).offset().top + $(self.container).height() - self.deviationTop;
					}
					$(self.treeContainer).css("left", left);
					$(self.treeContainer).css("top", top);
				break;
		...
		self.createTree = function() {
			...
			if(self.cls || self.style) {
				...
				$(self.treeContainer).css("height", "auto");
			...
			self.setPosition();
			$(self.parentContainer).append(self.treeContainer);
			if(self.data) {
				self.treeObj = base.tree({
	...
	base.datatables = function(option) {
		...
		self.selectType = option.selectType?option.selectType:null;
		self.selectAttribute = option.selectAttribute?option.selectAttribute:null;
		self.hasCheckboxSkin = option.hasCheckboxSkin==false?false:true;
		self.tempOption = {};
		self.nodataString = "<span style='color:#ccc'>暂无相关数据</span>";
		self.pageRedirect = option.pageRedirect==true?true:true;
		self.pageRedirectContainer = null;
		if(self.nodataIcon){
		...
		self.create = function() {
			if(self.container && self.option) {
				...
				if(self.option.ajax){
					self.option.ajax.xhrFields = {"withCredentials":true};
				}
				
					
					
				for(var key in self.option){
					switch(key){
						case "drawCallback":
							self.tempOption.drawCallback = function(setting){
								self.option.drawCallback(setting);
								if(self.callback){
									self.callback(setting);
								}
								if(self.selectType&&self.selectType=="checkbox"){
									var selectAllObj = $(self.container).find("thead input[name='cball']");
									var selectItem = $(self.container).find("input[name='cb']");
									if(self.hasCheckboxSkin){
										base.form.checkboxSkin({
											checkbox: $(self.container).find("input[type='checkbox']")
										});
									}
									
									base.selectAll(selectAllObj,selectItem,null,true);
								}
							}
						break;
						
						case "columns":
							self.tempOption.columns = [];
							$(self.option.columns).each(function(i,o){
								self.tempOption.columns.push(o);
							});
							if(self.selectType){
								
								self.tempOption.columns.unshift({"data": "id","sWidth":"5%","type":"checkbox"});
							}
						break;
						
						case "columnDefs":
							self.tempOption.columnDefs = [];
							$(self.option.columnDefs).each(function(i,o){
								var obj = {};
								for(var k1 in o){
									if(k1=="targets"){
										var n = o[k1];
										if(self.selectType){
											if(self.selectType=="checkbox"||self.selectType=="radio"){
												obj[k1] = n+1;
											}
								...
								self.tempOption.columnDefs.push(obj);
							});
							if(self.selectType){
								if(self.selectType=="checkbox"||self.selectType=="radio"){
									self.tempOption.columnDefs.push(
										{"render":function(data,type,row,meta){
											var ats ="";
											if(self.selectAttribute){
												$(self.selectAttribute.split(",")).each(function(i,o){
													ats+=" "+o+"='"+row[o]+"'";
												});
											}
		                					return "<input  type='"+self.selectType+"' name='cb' value='"+row.id+"' "+ats+"/>";
		              					}, 
		               					"targets":0 
		          						}
									);
						...
						default:
							self.tempOption[key] = self.option[key];
						break;
					}
				}
				if(self.selectType){
					switch(self.selectType){
						case "checkbox":
							if($(self.container).find("thead input[name='cball']").length==0){
								$(self.container).find("thead tr").prepend("<th><input type='checkbox' name='cball'/></th>");
							}else{
								$(self.container).find("thead input[name='cball']").prop("checked",false);
							}
						break;
				...
				self.grid = $(self.container).DataTable(self.tempOption);
			}
	...
	/**html、xml 换行**/
	base.wrap = function(data,container){
