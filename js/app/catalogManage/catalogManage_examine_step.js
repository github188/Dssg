	...
	var setContent = function(){
		...
		base.ajax({
			...
			success:function(d){
				...
				if(data.filePath){//图片
					$(".ui-page-headingImage").attr("src",$.path1+data.filePath)
				}else if(data.pictureUrl){
					$(".ui-page-headingImage").attr("src",$.path1+data.pictureUrl)
				}else{
					$(".ui-page-headingImage").attr("src","../images/default.png")
				}
//				var pictureUrl = data.filePath?data.filePath:data.pictureUrl;
//				$(".ui-page-headingImage").attr("src",$.path1+pictureUrl)
			}
		});
	}
