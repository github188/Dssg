...
//		时间控件
	...
	//获取资源名称
	var getSourceName = function(resType,subDeviceId){ 
		var params={resType:resType?resType:"1"};//resType:1数据库2文件3 api;
		if(subDeviceId){
			params = $.extend(params,{equipment:subDeviceId})
		}
		base.ajax({
			type:"post",
...
//		var params={resourceId:"0",equipment:"0",startTime:2017-08-04,endTime:2017-09-04,resType:1,model:2};//resType:1数据库2文件3 api;
		...
		var paramsA = $.extend(params,{resType:resType,model:2});
		var paramsStr="";
		//对象转换字符串.
	...
	var setGridButton = function(){
		...
		//订阅设备点击事件
		$("#search-form .subDevice").on("change",function(){
			var equipment = $(this).val();
			var resType=$(".ui-grid-buttonbar .ui-grid-linkGroup li.active").attr("key");
			if($("#search-form .sourceName").val() =="0"){		
				getSourceName(resType,equipment)
				if($("#search-form .subDevice").val()=="0"){
